...
#from network_cam import vc_net
from tensorflow.contrib.slim.nets import resnet_v1
from sklearn.externals import joblib
import numpy as np
import tensorflow as tf
import tensorflow.contrib.slim as slim
from network_cam import DeepMIL
from network_cam import DeepRR
import math
...
def read_and_decode(filename, is_train=True):
    ...
    #num=45
    #img = tf.contrib.image.rotate(img, angles= num * math.pi / 180,interpolation='BILINEAR')
    #num=180*np.random.random_sample(size=None)
    #img = tf.contrib.image.rotate(img, angles= num * math.pi / 180,interpolation='BILINEAR')
...
def get_label_pred(ckpt_num, is_save=False):
    ...
    instance_pooling, conv6, attention = DeepRR(fmap1, is_training=False)
